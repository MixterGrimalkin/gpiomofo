...
public class ZapperScenario extends Scenario {
    ...
    public void setupTriggers() {
        ...
        Trigger three = triggers.http("Three");
        Trigger notOne = triggers.gpio("Not-One", 0, PULL_DOWN, false);
        Trigger notTwo = triggers.gpio("Not-Two", 3, PULL_DOWN, true);
        Trigger notThree = triggers.invert("Not-Three", triggers.get("Three"));

        triggers.composite("None",          notOne, notTwo, notThree);
        triggers.composite("Just-One",      one,    notTwo, notThree);
        triggers.composite("Just-Two",      notOne, two,    notThree);
        triggers.composite("Just-Three",    notOne, notTwo, three);
        triggers.composite("One-Two",       one,    two,    notThree);
        triggers.composite("One-Three",     one,    notTwo, three);
        triggers.composite("Two-Three",     notOne, two,    three);
